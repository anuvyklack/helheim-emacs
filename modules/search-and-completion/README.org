...
- =helheim-embark= ::
  Configures the [[https://github.com/oantolin/embark][Embark]] package. See below for more details.
...
- =i= ::
  In practically all buffers with search result: ~grep-mode~, ~deadgrep-mode~,
  ~occur-mode~ you can switch to Insert state for editing the results files
  directly from a search results buffer. To switch back to Motion state use
  =ZZ= or =zx= or =C-c C-c=.

  The only place where it doesn't work is buffer with Xref results
